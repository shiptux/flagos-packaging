...
Produced binary: **python3-flag-attention** (≈23 KB .deb, ≈64 KB .rpm).

## What changed

- `packaging/debian/{control,rules,changelog,copyright,source/format}` — Debian source-format-3.0-native packaging.
- `packaging/rpm/specs/flag-attention\.spec` — RPM spec using `pyproject-rpm-macros`.
...
- `triton` is required at runtime (declared as Recommends via `${python3:Depends}` from pyproject). Users on systems without `python3-triton` in the apt archive should install it from upstream PyPI before running flag_attn kernels.
