...
Produced binary: **python3-flagquantum** (≈36 KB .deb, ≈92 KB .rpm).

## What changed

- `packaging/debian/{control,rules,changelog,copyright,source/format}` — Debian source-format-3.0-native packaging.
- `packaging/rpm/specs/flagquantum\.spec` — RPM spec using `pyproject-rpm-macros`.
...
- Runtime deps: torch + numpy. No triton dependency (FlagQuantum's kernels are pure PyTorch, not Triton-based).
