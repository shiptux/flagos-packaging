...
Produced binary: **python3-flagtensor** (≈32 KB .deb, ≈116 KB .rpm).

## What changed

- `packaging/debian/{control,rules,changelog,copyright,source/format}` — Debian source-format-3.0-native packaging.
- `packaging/rpm/specs/flagtensor\.spec` — RPM spec using `pyproject-rpm-macros`.
...
- Runtime deps: torch + triton + matplotlib + openpyxl (the latter two for benchmark / report rendering, optional in practice).
