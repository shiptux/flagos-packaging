...
| 12 | flagos-ai/libtriton_jit | pr/packaging           | [#24](https://github.com/flagos-ai/libtriton_jit/pull/24)  | **MERGED** 2026-07-13 | merged into `multi-backend`, which master has since absorbed — packaging is effectively on master. Follow-ups all retargeted/pinned to master, CI green, maintainer (tengqm) pinged 2026-08-20: #47 (lgtm'd 7-25, asked to merge), #48/#49 (system-dep packaging fixes), #58 (multiarch layout), #61 (per-backend packaging); #62 CLOSED — core perf duplicated #22 (Artlesbol, 3 months older), json relax extracted as #64; FlagFFT submodule re-pinned to #64's commit f84677b |
...
| 17 | flagos-ai/FlagCX       | pr/openeuler-rpm        | [#549](https://github.com/flagos-ai/FlagCX/pull/549)       | OPEN, no review | openEuler 24.03 NVIDIA RPM matrix (same template as FlagSparse#29); built+verified on A100 with NCCL 2.31 (unpinned, tracks current releases); incl. spec changelog-order + comment-macro fixes; CI blocked on maintainer approving workflow-file changes from fork |
| 18 | flagos-ai/FlagAudio     | fix/gain-identity-and-torchaudio-imports | [#4](https://github.com/flagos-ai/FlagAudio/pull/4) | OPEN, no review | code-bugfix PR split out of the packaging work: gain_db identity check (1.0→0.0) + top-level torchaudio imports deferred; A100-verified vs torchaudio reference |
